...
async def generate_command(
    ...
    Converts natural language into executable FFmpeg commands for
    video/audio processing, format conversion, and media manipulation.
    
    Args:
        request: Natural language task description (max 10,000 chars).
        return_raw: Return raw CommandPlan object (True) or executable 
                   commands (False). Default: False.
        assume_yes: Auto-confirm FFmpeg prompts (-y flag). Default: True.
        workspace_dir: Directory containing media files. Default: current dir.
    
    Returns:
        Executable FFmpeg commands list or raw CommandPlan object.
    
    Raises:
        ValueError: Empty or oversized request.
        TranslationError: Invalid media operation.
...
async def scan_workspace(directory: Optional[str] = None) -> Dict[str, Any]:
    ...
    Discovers and catalogs media files recursively for use in
    command generation.
    
    Args:
        directory: Directory to scan (absolute or relative path).
                  Default: current working directory.
    
    Returns:
        Dict with categorized media files:
        - 'videos': Video files (mp4, avi, mkv, mov, webm)
        - 'audios': Audio files (mp3, wav, flac, aac, ogg)
        - 'images': Image files (jpg, png, gif, bmp, webp)
        - 'subtitle_files': Subtitle files (srt, vtt, ass, sub)
        - 'total_files': Total count
        - 'scan_path': Scanned directory
    """
...
Examples:
  ...
  %(prog)s --http --host 0.0.0.0    # HTTP transport on all interfaces
    ...
    transport_group.add_argument("--http", action="store_true", help="Use Streamable HTTP transport")
    ...
    parser.add_argument("--port", type=int, default=3001, help="Port to bind to (default: %(default)s)")
